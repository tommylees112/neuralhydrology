...
def subset_data_dicts(predictions: Dict[str, Tensor], predict_last_n: int = 1) -> Dict[str, Tensor]:
...
def convert_to_xarray(cfg: Config, output: DefaultDict[str, Tensor], key: str, basin: str) -> xr.Dataset:
    data = output[key].detach().cpu().numpy()
    date_range = pd.date_range(start=cfg.test_start_date, end=cfg.test_end_date, freq="1D")

    return xr.Dataset(
        {"c_n": (("date", "station_id", "dimension"), data)},
        coords={"date": date_range, "station_id": [basin], "dimension": np.arange(data.shape[-1])}
    )


def _extract_cell_state_from_model_forward_pass(cfg: Config, run_dir: Path, model: BaseModel) -> List[xr.Dataset]:
    ...
    for basin in pbar:
        ...
        with torch.no_grad():
            for data in loader:
                ...
                subset_preds = subset_data_dicts(predictions, predict_last_n=cfg.predict_last_n)
                ...
                for key in [k for k in subset_preds.keys()]:
                    output[key] = torch.cat((output[key], subset_preds[key].detach().cpu()), 0)
